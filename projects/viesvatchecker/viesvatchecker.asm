...
request:                db 'POST /taxation_customs/vies/vatResponse.html HTTP/1.1',13, 10
                        ...
                        ; memberStateCode=XX&number=1234567890&traderName=&traderStreet=&traderPostalCode=&traderCity=&requesterMemberStateCode=&requesterNumber=&action=check&check=Verify
                        
                        ;;;  db 'memberStateCode=XX&number=1234567890&action=check&check=Verify'
                        ; will return : 
                        ; Yes, valid VAT number
                        ;
                        ; Member State               XX
                        ; VAT Number                 XX 1234567890
                        ; Date when request received 2015/02/17 08:53:02
                        ; Name                       COMPANY NAME OMITTED
                        ; Address                    123 MAIN STREET
                        ;                            0000 CITYNAME
                        db 'memberStateCode=XX&number=9876543210&action=check&check=Verify'
                        ; will return :
                        ; Yes, valid VAT number
                        ;
                        ; Member State   XX
                        ; VAT Number     XX 9876543210
...
; fill in sock_addr structure (on stack)
        ...
        mov     r8, 0x67774393           ; VIES VAT checker IP 147.67.119.103 (ec.europa.eu); 100007F (IP address : 127.0.0.1)
